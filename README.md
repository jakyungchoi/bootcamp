...
- 사이트 전역 설정 — 로고, 사이트 이름, 홈 화면 히어로 제목/부제목/이미지, **헤더 메뉴 ·
  홈 화면 카드 항목**(자유롭게 추가/삭제/수정 가능 — 4개로 고정되어 있지 않습니다)
...
- **새 탭 추가 버튼** — 완전히 새로운 탭(페이지)을 만듭니다. 이름을 입력하면 바로 편집
  ...
  삭제**할 수도 있습니다.
- 새로 만든 탭을 헤더 메뉴에도 넣고 싶다면, "사이트 전역 설정 > 헤더 메뉴 · 홈 화면 카드"
  에서 항목을 추가하고 링크 주소에 `/pages/주소`를 입력해주세요.
